...
ksemilla  <- 102451  #poner aqui la PRIMERA de sus cinco semillas
...
modelo  <- lgb.train( data= dtrain,
                      param= list( objective=        "binary",
                                   max_bin=              31,
                                   learning_rate=        0.0101028520398027,
                                   num_iterations=      492,
                                   num_leaves=          432,
                                   min_data_in_leaf=   2490,
                                   feature_fraction=      0.678809965824984,
...
entrega  <- as.data.table( list( "numero_de_cliente"= dapply[  , numero_de_cliente],
                                 "Predicted"= as.integer(prediccion > 0.016324422381903)   ) ) #ATENCION  no es  1/60
...
archivo_importancia  <- "552_importancia_017.txt"
